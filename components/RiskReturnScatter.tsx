  ...
  return (
    <div className="w-full h-full min-h-[400px] glass-panel p-4 rounded-xl flex flex-col relative group">
      <div className="flex justify-between items-start mb-4 z-10">
        <div>
          <h3 className="text-sm font-medium text-neutral-200">Portfolio Efficiency</h3>
          <p className="text-xs text-neutral-500">Risk (Beta) vs Expected Return</p>
        </div>
        <button
        ...
        {showInfo && (
          ...
          >
            <div className="flex justify-between items-start">
                <h4 className="text-sm font-bold text-emerald-400">Understanding Risk vs Return</h4>
            ...
            <div className="text-xs text-neutral-300 space-y-2 leading-relaxed overflow-y-auto">
              <p>
                This chart helps you evaluate if you are being compensated enough for the risk you are taking.
              </p>
              <ul className="list-disc pl-4 space-y-1 text-neutral-400">
                <li><strong className="text-white">Vertical Axis (Return):</strong> Higher is better. It estimates total return (Yield + Growth).</li>
                <li><strong className="text-white">Horizontal Axis (Risk/Beta):</strong> Measures volatility relative to the market.
              ...
              <p className="mt-2 text-emerald-300 italic border-l-2 border-emerald-500 pl-2">
                Goal: Find assets in the <strong>Top-Left</strong> (High Return, Low Risk). Avoid the Bottom-Right.
              </p>
            </div>
          </motion.div>
        )}
      ...
      <div className="flex-1 min-h-0 relative">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 10 }}>
            ...
            >
              <Label value="Risk (Beta)" offset={0} position="bottom" fill="#525252" fontSize={10} />
            </XAxis>
            <YAxis
              type="number"
              dataKey="y"
              ...
              unit="%"
            >
              <Label value="Return (%)" angle={-90} position="insideLeft" fill="#525252" fontSize={10} />
            </YAxis>
                ...
                if (active && payload && payload.length) {
                  ...
                  return (
                    <div className="bg-stone-950/90 backdrop-blur-md border border-white/10 p-3 rounded-lg shadow-xl min-w-[150px]">
                      <p className="font-bold text-white mb-2 border-b border-white/10 pb-1">{d.ticker}</p>
                      <div className="text-xs space-y-1.5">
                        <div className="flex justify-between gap-4">
                          <span className="text-neutral-400">Risk (Beta):</span>
                          <span className={d.x > 1.2 ? 'text-rose-400' : d.x < 0.8 ? 'text-emerald-400' : 'text-amber-400'}>
                            {d.x.toFixed(2)}
                          </span>
                        </div>
                        <div className="flex justify-between gap-4">
            ...
            <ReferenceLine x={1} stroke="#525252" strokeDasharray="3 3">
               <Label value="Market (1.0)" position="insideTopRight" fill="#525252" fontSize={10} offset={10} className="hidden sm:block" />
            </ReferenceLine>

             {/* Low Risk Zone Indicator (Arbitrary visual guide) */}
             <ReferenceLine x={0.8} stroke="#10b981" strokeOpacity={0.2} strokeDasharray="5 5" />
