...
import { BrainCircuit, TrendingUp, ShieldCheck, Scale, ArrowRight, Zap, Target, Lock, Calculator, ChevronRight, Plus, Equal } from 'lucide-react';
...
export default function AlgorithmExplainer() {
  ...
  const [simulationState, setSimulationState] = useState<'IDLE' | 'SIMULATING' | 'CALCULATING' | 'SELECTED'>('IDLE');
                  ...
                  >
                    ...
                    <div className="relative group">
                      ...
                      <div className="relative bg-black/80 border border-white/10 px-10 py-8 rounded-2xl shadow-2xl backdrop-blur-md flex items-center gap-4 text-3xl md:text-4xl font-serif text-white">
                        <span
                          className={cn("transition-colors cursor-help italic font-bold", hoveredTerm === 'U' ? "text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.5)]" : "text-white")}
                          onMouseEnter={() => setHoveredTerm('U')}
                          onMouseLeave={() => setHoveredTerm(null)}
                        >
                          U
                        </span>
                        <span className="text-neutral-600 text-2xl mx-1">=</span>
                        <div className="flex items-center group/return">
                           <span
                            className={cn("transition-colors cursor-help italic", hoveredTerm === 'mu' ? "text-blue-400 drop-shadow-[0_0_8px_rgba(96,165,250,0.5)]" : "text-neutral-200")}
                        ...
                        <span className="text-neutral-600 text-2xl mx-2">-</span>
                        <div className="flex items-center group/risk">
                           <span
                            className={cn("transition-colors cursor-help italic font-bold", hoveredTerm === 'lambda' ? "text-rose-400 scale-110 drop-shadow-[0_0_8px_rgba(251,113,133,0.5)]" : "text-rose-400")}
                           ...
                           <span
                            className={cn("ml-3 transition-colors cursor-help italic", hoveredTerm === 'sigma' ? "text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]" : "text-neutral-200")}
                           ...
                           >
                             <div className="flex flex-col items-center gap-1">
                                ...
                                <span className="text-xs text-neutral-400">
                                  {hoveredTerm === 'U' && "Higher is better. Balancing greed vs. fear."}
                                  {hoveredTerm === 'mu' && "Weighted sum of asset returns."}
                                  {hoveredTerm === 'lambda' && "Penalty multiplier for volatility."}
                                  {hoveredTerm === 'sigma' && "Statistical measure of portfolio fluctuation."}
                                </span>
                             </div>
                             {/* Triangle arrow */}
                    ...
                    <div className="w-full max-w-sm bg-white/5 border border-white/10 rounded-xl p-6 flex flex-col gap-6 backdrop-blur-sm">
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-neutral-300 font-medium">Risk Aversion (λ)</span>
                        <span className="font-mono text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded">{lambda.toFixed(1)}</span>
                      </div>
                      <input
                        type="range"
                        min="0.5"
                      ...
                      <div className="flex gap-2 h-32 items-end pt-4 border-b border-white/10 pb-0">
                         {/* Hypothetical Assets */}
                         {[
                           { name: 'High Growth', base: 80, risk: 5, color: 'bg-blue-500' },
                           { name: 'Balanced', base: 50, risk: 0.5, color: 'bg-purple-500' },
                           { name: 'Conservative', base: 20, risk: -4, color: 'bg-emerald-500' } // Negative risk factor here implies safety benefit relative to lambda
                         ].map((asset, i) => (
                           <div key={i} className="flex-1 flex flex-col justify-end h-full gap-2 group relative">
                             <motion.div
                               className={cn("w-full rounded-t-sm opacity-80 group-hover:opacity-100 transition-opacity relative", asset.color)}
                               animate={{ height: `${Math.max(5, asset.base - (lambda * (asset.risk + 5)))}%` }} // Simplified math for demo
                             ...
                             >
                                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-black/80 text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none border border-white/10 z-10">
                                  Score: {Math.max(0, Math.round(asset.base - (lambda * (asset.risk + 5))))}
                                </div>
                             </motion.div>
                             <div className="text-[10px] text-neutral-500 text-center font-medium truncate w-full">{asset.name}</div>
                           </div>
                         ))}
                      </div>
                      <div className="text-xs text-neutral-500 text-center italic">
                        As λ increases, risky assets (blue) lose utility score faster than safe assets (green).
                      </div>
                    </div>
                  </motion.div>
                )}
                ...
                {activeStep === 1 && (
                  ...
                  >
                    ...
                    <div className="bg-black/60 border border-white/10 px-6 py-4 rounded-xl flex items-center gap-3 text-lg font-serif text-neutral-300">
                      ...
                      <span>=</span>
                      <span>U(w + δ<sub className="text-xs">i</sub>)</span>
                      <span>-</span>
                      <span>U(w)</span>
                    </div>
                    ...
                    <div className="w-full bg-white/5 rounded-2xl p-6 border border-white/10 min-h-[280px] flex flex-col justify-between">
                       <div className="flex justify-between items-end gap-2 h-40 mb-6 relative">
                          ...
                          <div className="absolute inset-x-0 bottom-[40%] border-t border-dashed border-white/20 flex items-end justify-end px-2">
                             <span className="text-[10px] text-neutral-500 mb-1">Current U</span>
                          </div>
                          ...
                          {['A', 'B', 'C', 'D', 'E'].map((ticker, i) => {
                             ...
                             return (
                               <div key={ticker} className="flex-1 flex flex-col items-center gap-2 relative group">
                                  {/* Bar Container */}
                                  <div className="w-full max-w-[40px] flex flex-col justify-end h-full relative">
                                     ...
                                     <motion.div
                                       className={cn(
                                         ...
                                         simulationState === 'SELECTED' && isWinner ? "bg-emerald-500" :
                                         simulationState === 'SELECTED' ? "bg-white/5 opacity-20" :
                                         "bg-blue-500"
                                       )}
                                       initial={{ height: 0, opacity: 0 }}
                                       animate={{
                                          height: simulationState !== 'IDLE' ? `${gain}%` : 0,
                                          opacity: simulationState !== 'IDLE' ? 1 : 0
                                       }}
                                       transition={{ delay: simulationState === 'SIMULATING' ? i * 0.1 : 0, duration: 0.4 }}
                                     >
                                        {/* Value Label */}
                                        {(simulationState === 'CALCULATING' || simulationState === 'SELECTED') && (
                                          <motion.div
                                            initial={{ opacity: 0, y: 10 }}
                                            animate={{ opacity: 1, y: 0 }}
                                            className={cn(
                                              "absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-mono font-bold",
                                              isWinner && simulationState === 'SELECTED' ? "text-emerald-400 scale-125" : "text-neutral-400"
                                            )}
                                          >
                                            +{gain}
                                          </motion.div>
                  ...
                  >
                     ...
                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
                       <div className="bg-white/5 border border-white/10 p-4 rounded-xl flex flex-col items-center justify-center gap-2">
                         <div className="text-xs text-neutral-400 uppercase tracking-widest font-bold">Integer Shares</div>
                         <div className="font-serif text-xl text-white">
                           n<sub className="text-sm">i</sub> ∈ ℤ<sup className="text-sm">≥0</sup>
                         </div>
                         <div className="text-[10px] text-neutral-500 text-center">Must be whole numbers {`{0, 1, 2...}`}</div>
                       </div>

                       <div className="bg-white/5 border border-white/10 p-4 rounded-xl flex flex-col items-center justify-center gap-2">
                         <div className="text-xs text-neutral-400 uppercase tracking-widest font-bold">Budget Constraint</div>
                         <div className="font-serif text-xl text-white flex items-center gap-2">
                           <span>Σ</span>
                           <span className="text-base">(n<sub className="text-xs">i</sub> × p<sub className="text-xs">i</sub>)</span>
                           <span>≤</span>
                           <span className="text-emerald-400">B</span>
                         </div>
                         <div className="text-[10px] text-neutral-500 text-center">Total cost cannot exceed cash</div>
                       </div>
                     </div>
                        ...
                        <div className="flex items-end gap-1 h-32 w-full justify-center">
                           ...
                           <div className="flex flex-col items-center gap-2 opacity-50 grayscale">
                              <div className="flex flex-col-reverse gap-1 relative">
                                {[1,2,3].map(i => (
                                  <div key={i} className="w-12 h-8 bg-white/10 border border-white/20 rounded flex items-center justify-center" />
                                ))}
                                {/* Fractional Part */}
                                <div className="absolute -top-5 w-12 h-4 bg-rose-500/20 border border-rose-500/50 rounded-t flex items-center justify-center">
                                   <span className="text-[9px] text-rose-400">0.42</span>
                                </div>
                              </div>
